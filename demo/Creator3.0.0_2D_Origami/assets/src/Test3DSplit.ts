...
import { _decorator, Component, MeshRenderer, primitives, utils, Vec3, v3, Quat, quat, systemEvent, SystemEventType, EventTouch, Touch, Camera, geometry, Line, mat4, Slider } from 'cc';
...
export class Test3DSplit extends Component {
    ...
    start() {
        ...
        systemEvent.on(SystemEventType.TOUCH_END, this.onTouchStart, this);
    }
    ...
    private reset() {
        ...
        this.cameraCom.node.parent!.eulerAngles = v3(0, 0, 0);
    }
    ...
    private rotateMesh(rad: number) {
        ...
        this._selectedPos.forEach((arr, i) => {
            ...
            // _temp_v3_1.set(this._gemotry.normals![0], this._gemotry.normals![0 + 1], this._gemotry.normals![0 + 2]);
            // this.rotateVector(_temp_v3_1, axiStart, axiEnd, rad);
            // this._gemotry.normals![i] = _temp_v3_1.x;
            // this._gemotry.normals![i + 1] = _temp_v3_1.y;
            // this._gemotry.normals![i + 2] = _temp_v3_1.z;
        })
        this.renderMesh();
    }
    ...
    private sliderCallback(slider: Slider, customEventData: string) {
        switch (customEventData) {
            case 'camera': {
                this.cameraCom.node.parent!.eulerAngles = v3(0, 0, slider.progress * 360 - 180);
                break
            }
            default: {
