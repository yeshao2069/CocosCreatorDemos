import { _decorator, Component,  Node, Prefab, Texture2D,instantiate, find,ImageAsset, game,  MeshRenderer,SkinnedMeshRenderer,resources } from 'cc';
...
export class Avatar extends Component {
    ...
    changeBodypart(part: number, suitId: string) {
        ...
        resources.load('Prefabs/' + partName, Prefab, (err, prefab) => {
            ...
            resources.load(`Textures/${albedoMapName}/texture`, Texture2D, (err, tex: Texture2D) => {
                if (tex) {
                    if (part == AvatarBodyparts.WEAPON) {
                        meshNode.getComponent(MeshRenderer)!.material!.setProperty('albedoMap', tex);
                    } else {
                        meshNode.getComponent(SkinnedMeshRenderer)!.material!.setProperty('albedoMap', tex);
                    }
                }
            });
        });
