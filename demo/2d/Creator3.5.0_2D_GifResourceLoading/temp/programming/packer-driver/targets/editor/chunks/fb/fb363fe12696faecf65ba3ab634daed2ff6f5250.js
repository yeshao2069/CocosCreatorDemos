  ...
  return {
    ...
    execute: function () {
      ...
      _export("GIF", GIF = class GIF {
        ...
        getTexture() {
          let index = 0; // console.log(this._info.frames,'info')
        ...
        decodeFrame(frame) {
          let imageData; // console.log(frame,'frame')
        ...
        date2SpriteFrame(data, width, height) {
          let texture = new Texture2D();
          let spriteFrame = new SpriteFrame(); // spriteFrame = find('Canvas/test').getComponent(Sprite).spriteFrame
          ...
          let imageAsset = new ImageAsset(image); // image.reset({
          //     _data: data._data,
          //     _compressed: false,
          //     width: width, 
          //     height: height,
          ...
          if (!GIFCache.instance) {
            ...
            if (JSB) {
              assetManager.parser.register('.gif', (file, options, onComplete) => {
                ...
                let buffer = jsb.fileUtils.getDataFromFile(file); // console.log(buffer,' >>> buffer')
...
//# sourceMappingURL=fb363fe12696faecf65ba3ab634daed2ff6f5250.js.map