import {_decorator, Component, Node, CCInteger, EventHandler, EventTouch, UITransform, CCFloat, Vec3, Button} from 'cc';
...
export class UIScrollSelect extends Component {
    ...
    _onTouchEnd(event: EventTouch) {
        if (this._touchId != null && event.touch != this._touchId) {
            console.log(" _onTouchEnd", event.touch, this._touchId);
            return;
        }
        this.isTouching = false;
